...
function Olympiad() {
  ...
  return (
    ...
    >
      ...
      <p className="block mb-8 sm:hidden font-light text-xl 2xl:text-2xl mulish">Салбарын гол олимпиадууд</p>
      {/* Big Banner */}
      {news.length > 0 && (
        (() => {
          const featuredBanners = news.filter(banner => banner.category.includes("feature") && banner.mainCategory == "olympiad");
      ...
      <p className="sm:mb-16 mb-6 hidden sm:block font-light text-xl 2xl:text-2xl mulish">Салбарын гол олимпиадууд</p>
      ...
      >
        <div className="flex sm:flex-row flex-col sm:gap-10 justify-center items-start sm:w-full sm:min-w-max">
          {news.filter((data) => data.mainCategory === "olympiad" && data.category.includes("feature")).reverse().slice(0, windowWidth < 640 ? 6 : news.length).map((data) => (
            <Card
              key={data.id}
              id={data.id}
              title={data.title}
      ...
      <div className="px-10 w-full flex-row  gap-10 justify-center items-start hidden sm:flex">
        {news.filter((data) => data.category.includes("meduushtei") && data.mainCategory === "olympiad").reverse().slice(-4).map((data) => (
          <Card
            key={data.id}
            id={data.id}
            title={data.title}
      ...
      <div className="sm:px-10 px-4 flex-wrap w-full flex flex-row sm:gap-10  justify-center items-start">
        {news.filter((data) => data.mainCategory === "olympiad")
